...
def get_spikes(spike_times, stim, trial_index, stimulus_type):
    ...
    spikes=np.array([spike_times[key] for key in spike_times.keys()])[:,trial_index*stimulus_length_dict[stimulus_type]:(trial_index+1)*stimulus_length_dict[stimulus_type]].T
    return spikes

def pipeline(stimulus_type='natural_movie_one_more_repeats', session_id=831882777):
    session=get_session(session_id)
    stim, spike_times = get_context(stimulus_type, session)
    n_trials=stimulus_repeats_dict[stimulus_type]
    for trial in range(n_trials):
        spikes=get_spikes(spike_times, stim, trial, stimulus_type)
