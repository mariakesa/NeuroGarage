...
for n in time_points_list:
    ...
    # 3.1. Extract the first 'n' time points
    data_subset = events[:, :n]  # Shape: (14062, n)
    
    # 3.2. Z-Score Standardization (Per Time Point)
    scaler = StandardScaler(with_mean=True, with_std=True)
    # StandardScaler expects features as columns, so transpose, fit_transform, then transpose back
    data_subset_zscored = scaler.fit_transform(data_subset.T).T
    
    # 3.3. Compute Pairwise Euclidean Distances
    print("Computing pairwise distances...")
    try:
        # pdist returns a condensed distance matrix
        distances = pdist(data_subset_zscored, metric='euclidean')
    except MemoryError:
    ...
    # 3.4. Calculate the Mean Distance and Standard Deviation
    avg_distance = distances.mean()
    std_distance = distances.std()
    average_distances.append(avg_distance)
    std_distances.append(std_distance)
    
    # 3.5. Calculate Coefficient of Variation (CV)
    cv = std_distance / avg_distance
    cv_distances.append(cv)
    
    end_time = time.time()
    elapsed = end_time - start_time
    print(f"Average pairwise distance (z-scored): {avg_distance:.4f}")
...
# Plot Mean Distance with Shaded ±1 Std Dev
...
# Titles and Labels
plt.title('Average Pairwise Distance & Coefficient of Variation vs. Number of Time Points (Z-Scored)', fontsize=16)
ax1.set_xlabel('Number of Time Points', fontsize=14)
ax1.set_ylabel('Average Euclidean Distance', color='b', fontsize=14)
